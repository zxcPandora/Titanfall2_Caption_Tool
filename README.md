...
- ✅ **BT 截断检查**：打包时，按照检测规则警告会触发引擎字幕截断问题的内容（见"已知问题"）
...
1. **含有 BT 的字幕出现 乱码\截断 问题**：引擎对 BT 名字（`BT-7274` 这类，BT + 连字符数字）之后、**跨空格**扫描到第一个标点或词尾的内容 **≥20 个 UTF-16 字符**的字幕，会从第 20 个字符起截断（40 字节样式化缓冲 = 19 字符显示 + null，属引擎格式化问题，非工具问题）。**标点是隔断**（`，。！？、；：…─—,.!?;:`）；**空格/连字符/数字是内容延续**（如 `泰坦 BT-7274 已就緒歡迎...` 无标点即触发）；说话者标签 `BT:`/`BT：` 不触发。工具打包时按此规则检查并自动警告。**规避：在含有 BT 名字的字幕后加标点隔断**（如 `BT-7274，`）或断句。
